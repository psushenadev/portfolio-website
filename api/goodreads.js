...
export default async function handler(req, res) {
  ...
  try {
    ...
    const books = items.slice(0, 5).map((book) => ({
      id: book.book_id ?? book.guid ?? crypto.randomUUID(),
      ...
      rating: Number(book.average_rating) || null,

      published: book.publication_year || null,
    }));
